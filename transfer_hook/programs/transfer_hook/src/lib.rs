...
declare_id!("uq9uRULyC6kQC3VLJGGRxGTPEqTy3QoFF71f1qSAsky");
...
pub mod transfer_hook {                                                  
    ...
    pub fn initialize_extra_account_meta_list(
      ...
      let external_program_id = spl_tlv_account_resolution::solana_pubkey::Pubkey::from(
    ctx.accounts.custom_program.key().to_bytes()
);
      ...
      &external_program_id, // Pass the actual Program ID
        false, // is_signer
        false, // is_writable (it's an executable program account)
    ...
    pub fn transfer_hook(ctx: Context<TransferHook>, amount: u64) -> Result<()> {
        ...
        if ctx.accounts.dividend_account.owner != ctx.accounts.custom_program.key {
        return Err(ProgramError::IllegalOwner.into());
    }

    // 2. Deserialize the data into your DividendPda struct layout
    ...
    let dividend_pda = DividendPda::try_deserialize(&mut &account_data[..])?;
        ...
        let dpt = dividend_pda.dividend_per_token;
...
pub struct InitializeExtraAccountMetaList<'info> {
    ...
    /// CHECK: External dividend program
...
pub struct TransferHook<'info> {
    ...
    #[account(
        mut,
        seeds = [b"dividend",mint.key().as_ref()],
        bump,
        seeds::program = custom_program.key(),
